    ...
    <table class="table table-striped table-hover">
      ...
      <tbody>
        <tr>
            ...
            <td>{{$refund->status}}</td>
            {{-- @php
                $refundStatusOptions = ['Cancel','Pending', 'Processing', 'Refunded'];
            @endphp
            <td>
              <form method="post" action="{{ route('refund.status.update', $refund->id) }}">
                  ...
                  <select name="status" onchange="this.form.submit()">
                      @foreach($refundStatusOptions as $option)
                          <option value="{{ $option }}" {{ $refund->status == $option ? 'selected' : '' }}>
                              {{ $option }}
                          </option>
                      @endforeach
                  </select>
          ...
          <div class="col-lg-12 col-lx-4">
            @if ($refund->order && $refund->order->cart)
                    <div class="form-group">
                        ...
                        <div class="row">
                            @foreach ($refund->order->cart as $item)
            ...
            <div class="refund-info">
              ...
              <table class="table">
                    ...
                    <tr>
                        <td>Refund Date</td>
                        <td> : {{$refund->created_at->format('D d M, Y')}} at {{$refund->created_at->format('g : i a')}} </td>
                    </tr>
                    @php
                        $refundStatusOptions = ['Pending', 'Approved', 'Rejected'];
                    @endphp
                    <tr>
                      <td>Refund Status</td>
                      <td> : {{$refund->status}}</td>
...
@push('styles')
<style>
    .refund-info,.shipping-info{
        background:#ECECEC;
        padding:20px;
    }
    .refund-info h4,.shipping-info h4{
